...
export const homePage = defineType({
  ...
  fields: [
    ...
    { name: 'heroVisible', type: 'boolean', title: 'Show Hero Section', initialValue: true, group: 'hero' },
    {
      ...
      type: 'array',
      title: 'Hero Slides',
      description: 'Add up to 3 slides for the homepage hero carousel. The first slide will be shown by default if only one is added.',
      of: [{ type: 'heroSlide' }],
      validation: (Rule) => Rule.max(3).error('Maximum 3 hero slides allowed.'),
      group: 'hero',
    },

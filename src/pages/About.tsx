...
export const About: React.FC = () => {
  ...
  const { data, isLoading } = useSanityPage<AboutPageData>(
    ABOUT_QUERY,
    aboutDefaults,
  );
  ...
  const videoFeatures = data.videoFeatures?.length
    ? data.videoFeatures
  ...
  const team = data.teamMembers?.length
    ? data.teamMembers
    : aboutDefaults.teamMembers;

  const filteredTeam =
    team?.filter((m: TeamMember) => m.category === activeTab) ?? [];
  ...
  return (
    <>
      <div className="flex flex-col min-h-screen">
        ...
        {/* Video Feature Section (After Who We Are) */}
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="px-4 py-4 shrink-0 text-left">
                        ...
                        <p
                          className={`text-xs mt-1 font-normal ${isSelected ? "text-brand-green" : "text-[#9DD4C8]"}`}
                        >
                          {member.role}
                        </p>
                      </div>
                  ...
                  <div className="flex-1 px-8 py-10 pr-14 overflow-y-auto space-y-4">
                    {selectedMember.bio
                      ...
                      .map((paragraph: string, i: number) => (
                        <p
                          key={i}
                          className="text-[13px] leading-relaxed text-[#4a7060] font-light"
                        >
                          {paragraph}
                        </p>
                      ))}
      ...
      <div className="py-16 sm:py-20 bg-gray-100 rounded-3xl md:rounded-[40px] px-6 sm:px-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-5xl mx-auto">
          {[1, 2, 3, 4].map((i) => (
            <div
              key={i}
              className="bg-white rounded-2xl p-6 sm:p-8 flex gap-5 items-start"
            >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-5 max-w-5xl mx-auto">
          {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
            <div
              key={i}
              className="rounded-2xl flex flex-col overflow-hidden bg-gray-100"
              style={{ aspectRatio: "9/13" }}
            >
